...
## Introduction
The aim of this project was to create two models using data about individual's labor market status. The first model was to predict whether or not a person would be working at the time of their baseline survey and the second was to predict if the person would be working for at least six months. The variables in the data set include the person's gender, date of birth and province collected at the baseline survey and also scores from various tests including cft, which tests a person's fluid intelligence.

## Data Cleaning for Model 1
The first thing we decided to do to merge the different scores data with the original data frame so we had more variables for each individual. We noticed that there were some ids that were repeated in the dataframes so we dropped them and got rid of the index column. We feature engineered the current and future financial situation columns into more useable, numeric variables and also created two age variables.
...
## Visualizations for Model 1
In the following graphs, we changed all the NA values to No/ False under the assumption that any person who would answer Yes/ True would have given an answer instead of leaving it blank.
```{r fig.width=8}
...
```
From this, we can see that CFT, COM, and NUM scores were pairwise highly correlated so we decided to only include CFT in the model.
...
```{r, echo=TRUE}
# model accuracy
data.frame('XGBTree'= mean(model1$results$Accuracy), 'GLM'= mean(model2$results$Accuracy))
```
...
```
We were able to make predictions using the first model and test set. However, when we tried to make the confusion matrix, we ran into an error that the arguments were not the same length. After some exploration, we found that we had about 4591 predictions while the test set had about 10301 observations. We concluded that since we used na.pass in the train control, the model skipped over any observations that had NA in one of the variables used to predict. 

## Debugging Confusion Matrix Error
In order to combat this, we attempted to filter the test data set to include just the observations that were complete with regards to the explantory variables and were able to construct a confusion matrix.
```{r}
...
Age and gender were the most significant variables of the five variables so as for interventions, different measures should be taken for those of different genders and of different ages. Males were more likely to be employed than females and the older a person was, the greater their likelihood of being employed. We also found that the higher a person's cft score, the more likely they were to be employed so one measure that can be taken to combat unemployment could be to improve people's fluid intelligence. 
